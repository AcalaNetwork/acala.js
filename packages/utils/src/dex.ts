 ...
 * @param {Fixed18} supplyPool - token amount in the supply pool (base tokens pool)
 * @param {Fixed18} targetPool - token amount in the target pool (other token pool)
 * @param {Fixed18} feeRate - swap fee rate
 * @param {Fixed18} [slippage=0] - swap slippage
 * @returns {Fixed18} return target token amount which can be received
 */
