...
export class LoanCalculator {
    ...
    debitAmount$;
    loanType;
    exchangeRate = FixedPointNumber.ZERO;
    constructor(params) {
        this.params = params;
    ...
    updateDebitValue(value) {
        return this.updateDebitAmount(value.div(this.exchangeRate));
    }
    get isReady$() {
        return this.params.globalLoan$.pipe(map(({ type }) => !!type), filter((i) => !!i));
    }
    async isReady() {
    ...
    get priceAtLiquidationRatio$() {
        return combineLatest({
            ...
            debit: this.debitValue$
        }).pipe(map(({ loanType, collateral, debit }) => {
            const { liquidationRatio } = loanType;
            if (debit.isZero())
                return new FixedPointNumber(Number.NEGATIVE_INFINITY);
    ...
    get priceAtRequiredRatio$() {
        return combineLatest({
            ...
            collateral: this.collateralAmount$,
            debit: this.debitValue$
        }).pipe(map(({ collateral, debit, loanType }) => {
    ...
    get maxGenerateDebitValue$() {
        ...
        }).pipe(map(({ globalLoan, collateralValue }) => {
    ...
    get maxRemainGenerateDebitValue$() {
        ...
        }).pipe(map(({ maxDebitValut, debitValue, type }) => {
    ...
    get requiredCollateralAmount$() {
        ...
        }).pipe(map(({ debitValue, price, type, minimumCollateral }) => {
    ...
    get canWithdrawCollateralAmount$() {
        ...
        }).pipe(map(({ collateral, requiredCollateral, type }) => {
    ...
    get status$() {
        return combineLatest({
            ...
            if (ratio.lt(type.requiredCollateralRatio.plus(STEP)) &&
