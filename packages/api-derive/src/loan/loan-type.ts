...
import { CurrencyId, Rate, Balance } from '@acala-network/types/interfaces/runtime';
 ...
 */
export function loanConstants (api: ApiInterfaceRx): DerivedLoanConstants {
  return {
 ...
 */
export function allLoansOverview (api: ApiInterfaceRx): () => Observable<DerivedLoanOverView[]> {
  return memo(() => {
    const currencyIds = getCollateralCurrencyIds(api);
